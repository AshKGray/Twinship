...
+ (instancetype) menuItem:(NSString *) title
                   ...
                   action:(SEL) action
{
    return [[MenuItem alloc] init:title
                              image:image
                             target:target
                             action:action];
}
...
- (void)performAction:(id)sender
{
    ...
    MenuItem *menuItem = _menuItems[button.tag];
...
- (UIView *) mkContentView
{
    ...
    UIFont *titleFont = [Menu titleFont];
    ...
    UIImage *selectedImage = [MenuView selectedImage:(CGSize){maxItemWidth, maxItemHeight + 2}];
    UIImage *gradientLine = [MenuView gradientLine: (CGSize){maxItemWidth - kMarginX * 4, 1}];
             ...
             inContext:(CGContextRef) context
{
    ...
    UIColor *tintColor = [Menu tintColor];
    if (tintColor) {
